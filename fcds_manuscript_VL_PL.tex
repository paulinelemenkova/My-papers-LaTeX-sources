...
The question of which software to use for geological data processing received from series of experiment in a soil laboratory is not an unambiguous one and has been discussed previously \cite{DrielDavis1989}, \cite{DavisHerzfeld1993}. 
...
In geotechnical engineering, methods include analysis of the physical and maechanical properties of soil factor to estimate suitability of the ground to loads and pressure. Laboratory tests of soil compressibility aimed to estimate bearing capacity of different types of soils and their physical and mechanical properties prior to road constructions \cite{Boldyrevetal2006}, \cite{Lemenkov2018f}, \cite{RidleyBurland}. In principle, three types of strength can be analyzed to measure soil structure: tensile strength, compressive strength, and shear strength. 
...
To work with LaTeX, the latest version of TeXShop Editor was installed and \LaTeX \space compiler was used. It contains necessary packages as well as additional required packages: https://www.latex-project.org.  The abbreviation 'pgf means portable graphics format'. In this study methodology includes applying \LaTeX \space packages \cite{Feuersaenger1}: \{tikz\}, \{tikz-3dplot\}, \{tikzpicture\}, \{pgfplot\}, \{filecontetns\}, \{spy\}. 3D visualization showing correlation between variables and descriptive statistical analysis based on the processing of a data array. 

\subsection{Data}

The study utilized a traditional data collection approach using laboratory equipment for compression strength measurements with the main modes of data collection being geological survey sampling, then applying methods of strengthening and stabilizing compressible soils \cite{CasagrandeLoughney1966}, \cite{Lemenkov2018e}, and consequent data analysis \cite{Aitchison1957}, \cite{Babuetal2005}, \cite{Lemenkov2018c}. Dataset in this study was received from experimental testing of the physical and mechanical properties of soils in laboratory. The approach was used to maintain objectivity in measuring equivalent cohesion of the soil samples. 
...
Mechanical properties of the frozen soil reflects closely ice behavior, which is filling most of the pore space in soil mass, despite unfrozen water presented in its structure. The pore ice is usually of polycrystalline type with a random crystal orientation \cite{Merriam1976}. In usual environmental conditions, the response of a soil to external stresses is governed by the motion of dislocations and can be represented by a power law creep equation of the Norton-Bailey type computed by the algebraic Formula \ref{eq:01}, \cite{Bratten2008}:

\begin{equation}
	\varepsilon_{cr}=A\sigma^{n}t^{m}
\end{equation}\label{eq:01}
...
Deformation in soil also include changes of shape (expressed as shear modulus G) which is calculated by Formula \ref{eq:02}, \cite{Millsetal1996}. Shear modulus of soil shows its rigidity is defined as the ratio of shear stress to the shear strain:

\begin{equation}
	G^{def}=\frac{\tau_{xy}}{\gamma_{xy}}=\frac{\frac{F}{A}}{\frac{\delta x}{l}}=\frac{Fl}{A\delta x}
\end{equation}\label{eq:02}
...
Compressive strength of a soil is expressed by value of uniaxial compressive stress reached when the material fails completely and is expressed by Formula \ref{eq:06}, \cite{Agterberg1964}, \cite{Agterberg1974}:  

\begin{equation}
	\sigma_{e}=\frac{F}{A_{0}}
\end{equation}\label{eq:06}
...
To do this, \LaTeX \space function 'boxplot' was used. According to the description of the code, the graph is displayed automatically (see illustrations below). The source code and the result of the statistical data distribution diagram are shown in Fig. 5. 
...
Automatization and computed based assistance in geosciences are rapidly developing and being continuously reported in existing works \cite{Schenke}, \cite{Lemenkova201995}, \cite{Lemenkova201994}
